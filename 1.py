...
url = 'https://warm-plateau-39485.herokuapp.com/meetings'
myMeeting = {'titre': "S2EE meeting",
             'days': 16,
             'month': 8,
             'heureInit': 17,
             'minuteInit': 00,
             'heureFin': 19,
             'minuteFin': 00,
             'members': ["7med", "Wisseme", "Mouna"],
             'pointsAbord': ["Le lieu", "La stratégie"],
             'rapporteur': "7med",
             'chaineId':  "#106546",
             'categNom': "S2EE 8) 8)",
             'coordId':  "#1504"}
...
async def on_message(message):
    ...
    if message.content.startswith('$meet'):
        ...
        j = msg.find('at')
        k = msg.find('till')
        x = requests.post(url, data=myMeeting)
        print(x.text)
        print(i)
        embedmeet = discord.Embed(
            title="Meeting", description=message.channel.name + ' meeting , react with ✅', color=0x00ff00)
        ...
        if msg.startswith('$c_log'):
            ...
            def check(m):
                if m.content != '**':
                    ...
                    embedVar.add_field(name='________________',
                                       value=log[i], inline=False)
                return True
